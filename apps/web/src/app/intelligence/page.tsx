...
export default function IntelligencePage() {
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="rounded-[18px] bg-ink p-5 text-white shadow-[0_12px_32px_rgba(0,0,0,0.24)] sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="mono text-[10px] uppercase tracking-[0.18em] text-white/50">02 · best next action</p><h2 className="mt-2 text-3xl text-white">{next.action.replace(/_/g, " ")}</h2></div><div className="rounded-xl border border-white/25 px-3 py-2 text-right"><div className="mono text-[9px] uppercase tracking-widest text-white/45">confidence</div><div className="serif text-2xl text-white">{Math.round(next.confidence * 100)}%</div></div></div><div className="my-6 h-px bg-white/20" /><p className="text-sm leading-relaxed text-white/75">{next.rationale}</p><div className="mt-6 rounded-xl border-l-2 border-rust bg-white/5 p-4"><p className="mono text-[10px] uppercase tracking-[0.16em] text-white/45">Voice agent may say</p><p className="mt-2 font-serif text-xl leading-relaxed text-white">“{next.verbalization_brief}”</p></div><button onClick={runDecision} disabled={status === "running"} className="btn mt-6 bg-rust">{status === "running" ? "DECIDING…" : "RUN NEXT DECISION →"}</button></section>
    </div>
    ...
    <section className="card p-5"><p className="mono text-[10px] uppercase tracking-[0.18em] text-charcoal/50">Non-negotiable guardrails</p><ul className="mt-3 grid gap-2 md:grid-cols-2">{strategy.guardrails.map((rule) => <li className="flex gap-2 text-sm text-charcoal/75" key={rule}><span className="text-red">×</span>{rule}</li>)}</ul></section>
  </div>;
}
function Metric({ label, value, note, tone }: { label: string; value: string; note: string; tone?: "rust" }) { return <div className="flex items-center justify-between gap-5 px-5 py-4"><div><dt className="mono text-[10px] uppercase tracking-[0.13em] text-charcoal/50">{label}</dt><dd className="mt-1 text-xs text-charcoal/65">{note}</dd></div><strong className="serif text-2xl" style={{ color: tone === "rust" ? "var(--red)" : "var(--ink)" }}>{value}</strong></div>; }
