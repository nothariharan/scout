...
export default function DiscoverPage() {
  ...
  return <div className="space-y-6">
    ...
    <details className="card p-4"><summary className="cursor-pointer font-medium text-ink">Add a consented test contact</summary><p className="mt-2 text-sm text-charcoal/65">Use this only for a number you are authorized to call, such as your own verified trial number. Scout requires E.164 format and still keeps outbound calling disabled until you explicitly enable it.</p><form onSubmit={addManualContact} className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><input value={manualName} onChange={(event) => setManualName(event.target.value)} required className="rounded-lg border border-white/75 bg-white/30 px-3 py-2 text-sm text-charcoal backdrop-blur" placeholder="Test contact name" /><input value={manualPhone} onChange={(event) => setManualPhone(event.target.value)} required pattern="\+[1-9][0-9]{7,14}" className="rounded-lg border border-white/75 bg-white/30 px-3 py-2 text-sm text-charcoal backdrop-blur" placeholder="+16055550123" /><button disabled={addingManual} className="btn justify-center">{addingManual ? "ADDING…" : "ADD TEST CONTACT"}</button></form></details>
