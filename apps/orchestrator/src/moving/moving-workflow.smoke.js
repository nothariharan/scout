...
const { server } = createServer({ discovery, callProvider: async (payload) => { dispatchedPayloads.push(payload); return { placed: false, reason: 'outbound calling is disabled' }; } });
...
const scope = JSON.parse(dispatchedPayloads[0].metadata.moving_scope);
assert.equal(scope.origin, 'Austin, Austin');
assert.equal(scope.destination, 'Round Rock, Round Rock');
assert.equal(dispatchedPayloads[0].metadata.move_date, '2026-08-01');
assert.equal(dispatchedPayloads[0].metadata.budget_ceiling, '2200');
